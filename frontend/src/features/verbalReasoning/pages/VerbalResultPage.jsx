// src/features/verbalReasoning/pages/VerbalResultPage.jsx
...
const VerbalResultPage = () => {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        setLoading(true);
        const res = await verbalService.getVerbalResult(attemptId);
        setResult(res.data);
        ...
        toast.error("Failed to load verbal results");
        navigate("/dashboard/verbal-reasoning");
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleReattempt = async () => {
    if (!result) return;
    try {
      const res = await verbalService.startVerbal({
        difficulty: result.difficulty,
        count: result.totalQuestions,
      });
      if (res.data?.attemptId) {
        navigate(`/dashboard/verbal-reasoning/${res.data.attemptId}`);
      }
    } catch (err) {
      toast.error("Failed to start new verbal session");
    }
  };
  ...
  return (
    <div className="flex min-h-[85vh] flex-col items-center px-4 py-6">
      ...
      >
        ...
        <VerbalResultView result={result} />

        <div className="mt-10 flex flex-col gap-3 sm:flex-row">
          <button
            onClick={handleReattempt}
            className="group flex-[1.4] flex items-center justify-center gap-2.5 rounded-2xl bg-brand-blue py-3.5 text-sm font-black text-white shadow-brand shadow-brand-blue/20 transition-all hover:bg-brand-blue-hover active:scale-[0.98]"
          >
          ...
          <button
            onClick={() => navigate(`/dashboard/verbal-reasoning/${attemptId}/review`)}
          ...
          <button
            ...
            className="flex items-center justify-center gap-2.5 rounded-2xl border border-status-error/20 bg-status-error/5 px-6 py-3.5 text-sm font-black text-status-error transition-all hover:bg-status-error/10 active:scale-[0.98]"
            title="Discard this session"
          >
            <Trash2 className="h-4 w-4" />
            <span className="sm:hidden lg:inline">Discard</span>
          </button>
        </div>

        <div className="mt-8 flex flex-col items-center gap-4">
          <button
            onClick={() => navigate("/dashboard/verbal-reasoning")}
      ...
      <DeleteConfirmationModal
        ...
        title="Discard Verbal Session?"
        description={`Are you sure you want to remove your session on "${result?.practiceName || 'Verbal Practice'}"?`}
        loading={isDeleting}
...
export default VerbalResultPage;
