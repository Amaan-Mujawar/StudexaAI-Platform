// src/features/verbalReasoning/pages/VerbalAttemptPage.jsx
...
import VerbalQuestionView from "../components/VerbalQuestionView.jsx";
import VerbalProgress from "../components/VerbalProgress.jsx";
import cx from "../../../utils/cx.js";

const VerbalAttemptPage = () => {
  ...
  const loadQuestion = async () => {
    try {
      setLoading(true);
      const res = await verbalService.getCurrentQuestion(attemptId);
      ...
      navigate(`/dashboard/verbal-reasoning/${attemptId}/result`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await verbalService.submitAnswer(attemptId, selectedIndex);
      setLocked(true);
      setFeedback(res.data.correct ? "correct" : "incorrect");

      if (res.data.completed) {
        toast.success("Verbal session completed!");
        setTimeout(() => {
          navigate(`/dashboard/verbal-reasoning/${attemptId}/result`);
        }, 1200);
      }
    } catch (err) {
      toast.error("Failed to submit verbal answer");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading && !question) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center p-4 text-center">
        ...
        <p className="text-lg font-black text-text-title">Preparing your text...</p>
        <p className="mt-2 text-sm font-semibold text-text-muted">Analyzing linguistic nuances</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-[85vh] flex-col items-center px-4 py-4 sm:py-6">
      <div className="w-full max-w-2xl">
        ...
        <VerbalProgress current={question.index} total={question.total} />
        ...
        {locked && feedback && (
          ...
          >
            {feedback === "correct" 
              ? "Eloquently done! Correct." 
              : "Not exactly, but linguistics is an art!"}
          </motion.div>
        )}
      </div>
    </div>
...
export default VerbalAttemptPage;
