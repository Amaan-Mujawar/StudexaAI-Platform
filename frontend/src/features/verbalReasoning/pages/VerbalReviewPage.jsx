// src/features/verbalReasoning/pages/VerbalReviewPage.jsx
...
import VerbalReviewCard from "../components/VerbalReviewCard.jsx";

const VerbalReviewPage = () => {
  ...
  useEffect(() => {
    const loadReview = async () => {
      try {
        setLoading(true);
        const res = await verbalService.getVerbalReview(attemptId);
        setReview(res.data);
        window.scrollTo({ top: 0, behavior: "smooth" });
      } catch (err) {
        toast.error("Failed to load verbal review");
    ...
    <div className="flex flex-col items-center px-4 py-4 sm:py-6">
      ...
      >
        <div className="mb-8 flex flex-col items-center text-center">
          <button
            onClick={() => navigate(`/dashboard/verbal-reasoning/${attemptId}/result`)}
          ...
          <h1 className="text-3xl font-black text-text-title tracking-tight">Review Results</h1>
          <p className="mt-2 text-[13px] font-semibold text-text-muted">
            Analyzing {review.questions?.length || 0} solutions from your verbal session.
          </p>
        </div>

        <div className="space-y-4">
          {review.questions?.map((q, idx) => (
            <VerbalReviewCard key={idx} index={idx} data={q} />
          ))}
        </div>
...
export default VerbalReviewPage;
