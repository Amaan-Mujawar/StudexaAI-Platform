// src/features/aiQuiz/pages/AiQuizAttemptPage.jsx
...
import AiQuizQuestionView from "../components/AiQuizQuestionView.jsx";
import AiQuizProgress from "../components/AiQuizProgress.jsx";
...
const AiQuizAttemptPage = () => {
  ...
  const loadQuestion = async () => {
    try {
      setLoading(true);
      const res = await quizService.getCurrentQuestion(attemptId);
      ...
      navigate(`/dashboard/ai-quiz/${attemptId}/result`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await quizService.submitAnswer(attemptId, selectedIndex);
      setLocked(true);
  ...
  if (loading && !question) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center p-4 text-center">
        ...
        <p className="mt-2 text-sm font-semibold text-text-muted">Analyzing your progress</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-[85vh] flex-col items-center px-4 py-4 sm:py-6">
      <div className="w-full max-w-2xl">
        ...
        <AiQuizProgress current={question.index} total={question.total} />
