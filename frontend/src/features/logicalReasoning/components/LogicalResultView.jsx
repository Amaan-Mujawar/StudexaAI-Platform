// src/features/logicalReasoning/components/LogicalResultView.jsx

import { Trophy, BarChart, Percent, Brain } from "lucide-react";
import cx from "../../../utils/cx.js";

const LogicalResultView = ({ result }) => {
    ...
    <div className="space-y-4">
      <div className="flex flex-col items-center justify-center p-6 text-center rounded-3xl border border-border bg-white shadow-card">
        ...
        <h2 className="text-xl font-black text-text-title mb-1">
          {percentage >= 80 ? "Grandmaster!" : percentage >= 50 ? "Solid Logic!" : "Keep Thinking!"}
        </h2>
        <p className="text-[13px] font-semibold text-text-muted max-w-sm">
          You solved {score} out of {total} logic puzzles correctly.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <ResultStat icon={<Brain className="h-4 w-4" />} label="Type" value="Logical" />
...
export default LogicalResultView;
