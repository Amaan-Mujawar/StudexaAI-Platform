// src/features/logicalReasoning/pages/LogicalReviewPage.jsx
...
import LogicalReviewCard from "../components/LogicalReviewCard.jsx";

const LogicalReviewPage = () => {
  ...
  useEffect(() => {
    const loadReview = async () => {
      try {
        setLoading(true);
        const res = await logicalService.getLogicalReview(attemptId);
        setReview(res.data);
  ...
  if (loading) {
    return (
      <div className="flex min-h-[80vh] flex-col items-center justify-center p-4 text-center">
        ...
        <p className="text-lg font-black text-text-title tracking-tight">Fetching Deductions...</p>
      </div>
    );
  }
    ...
    <div className="flex flex-col items-center px-4 py-4 sm:py-6">
      ...
      >
        <div className="mb-8 flex flex-col items-center text-center">
          <button
            onClick={() => navigate(`/dashboard/logical-reasoning/${attemptId}/result`)}
          ...
          <h1 className="text-3xl font-black text-text-title tracking-tight">Logic Review</h1>
        ...
        <div className="space-y-4">
          {review.questions?.map((q, idx) => (
            <LogicalReviewCard key={idx} index={idx} data={q} />
          ))}
        </div>
...
export default LogicalReviewPage;
