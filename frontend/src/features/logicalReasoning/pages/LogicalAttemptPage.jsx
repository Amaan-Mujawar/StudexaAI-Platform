// src/features/logicalReasoning/pages/LogicalAttemptPage.jsx
...
import LogicalQuestionView from "../components/LogicalQuestionView.jsx";
import LogicalProgress from "../components/LogicalProgress.jsx";
import cx from "../../../utils/cx.js";

const LogicalAttemptPage = () => {
  ...
  const loadQuestion = async () => {
    try {
      setLoading(true);
      const res = await logicalService.getCurrentQuestion(attemptId);
      ...
      navigate(`/dashboard/logical-reasoning/${attemptId}/result`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const res = await logicalService.submitAnswer(attemptId, selectedIndex);
      setLocked(true);
      setFeedback(res.data.correct ? "correct" : "incorrect");

      if (res.data.completed) {
        toast.success("Logic set completed!");
        setTimeout(() => {
          navigate(`/dashboard/logical-reasoning/${attemptId}/result`);
        }, 1200);
      }
    } catch (err) {
      toast.error("Failed to submit logic answer");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  if (loading && !question) {
    return (
      <div className="flex min-h-[70vh] flex-col items-center justify-center p-4 text-center">
        ...
        <p className="text-lg font-black text-text-title">Preparing your puzzle...</p>
        <p className="mt-2 text-sm font-semibold text-text-muted">Analyzing deductive patterns</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex min-h-[85vh] flex-col items-center px-4 py-4 sm:py-6">
      <div className="w-full max-w-2xl">
        ...
        <LogicalProgress current={question.index} total={question.total} />
        ...
        <div className="mt-8 flex justify-center">
          {!locked ? (
            ...
            >
              ...
              {submitting ? "Checking..." : "Submit Logic"}
            </button>
          ) : (
            <button
        ...
        {locked && feedback && (
          ...
          >
            {feedback === "correct" 
              ? "Flawless deduction! Your logic is sharp." 
              : "Interesting attempt, but the logic points elsewhere."}
          </motion.div>
        )}
      </div>
    </div>
...
export default LogicalAttemptPage;
