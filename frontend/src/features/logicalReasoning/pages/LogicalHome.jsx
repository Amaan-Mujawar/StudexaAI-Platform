// src/features/logicalReasoning/pages/LogicalHome.jsx
...
import LogicalGenerator from "../components/LogicalGenerator.jsx";
...
const LogicalHome = () => {
  ...
  const { generating, createLogical } = useLogicalReasoning();

  const handleGenerate = async (params) => {
    try {
      const data = await createLogical(params);
      if (data?.attemptId) {
        toast.success("Logic puzzles ready!");
        navigate(`/dashboard/logical-reasoning/${data.attemptId}`);
      }
    } catch (err) {
      toast.error(err?.message || "Failed to start logic session");
    }
  };
      ...
      >
        <div className="mb-6 text-center">
          ...
          <p className="mt-2 text-sm font-semibold text-text-muted">
            Test your deductive powers with adaptive problem sets designed to sharpen your thinking.
          </p>
        </div>
