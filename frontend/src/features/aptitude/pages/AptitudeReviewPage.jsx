// src/features/aptitude/pages/AptitudeReviewPage.jsx
...
import AptitudeReviewCard from "../components/AptitudeReviewCard.jsx";

const AptitudeReviewPage = () => {
  ...
  useEffect(() => {
    const loadReview = async () => {
      try {
        setLoading(true);
        const res = await aptitudeService.getAptitudeReview(attemptId);
        setReview(res.data);
    ...
    <div className="flex flex-col items-center px-4 py-4 sm:py-6">
      ...
      >
        <div className="mb-8 flex flex-col items-center text-center">
          <button
            onClick={() => navigate(`/dashboard/aptitude/${attemptId}/result`)}
          ...
          <h1 className="text-3xl font-black text-text-title tracking-tight">Review Solutions</h1>
        ...
        <div className="space-y-4">
          {review.questions?.map((q, idx) => (
            <AptitudeReviewCard key={idx} index={idx} data={q} />
          ))}
        </div>
...
export default AptitudeReviewPage;
