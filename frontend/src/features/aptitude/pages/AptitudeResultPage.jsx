// src/features/aptitude/pages/AptitudeResultPage.jsx
...
import AptitudeResultView from "../components/AptitudeResultView.jsx";
...
const AptitudeResultPage = () => {
  ...
  useEffect(() => {
    const loadResult = async () => {
      try {
        setLoading(true);
        const res = await aptitudeService.getAptitudeResult(attemptId);
        setResult(res.data);
  ...
  const handleReattempt = async () => {
    if (!result) return;
    try {
      const res = await aptitudeService.startAptitude({
        difficulty: result.difficulty,
        count: result.totalQuestions,
      });
      if (res.data?.attemptId) {
        navigate(`/dashboard/aptitude/${res.data.attemptId}`);
      }
    } catch (err) {
  ...
  return (
    <div className="flex min-h-[85vh] flex-col items-center px-4 py-6">
      ...
      >
        ...
        <AptitudeResultView result={result} />
        ...
        <div className="mt-10 flex flex-col gap-3 sm:flex-row">
          ...
          <button
            onClick={() => navigate(`/dashboard/aptitude/${attemptId}/review`)}
      ...
      <DeleteConfirmationModal
        ...
        description={`Are you sure you want to remove your session on "${result?.practiceName || 'Aptitude'}"?`}
        loading={isDeleting}
