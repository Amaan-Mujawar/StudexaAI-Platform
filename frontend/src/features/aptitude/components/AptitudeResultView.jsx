// src/features/aptitude/components/AptitudeResultView.jsx

import { Trophy, BarChart, Percent, LayoutGrid } from "lucide-react";
import cx from "../../../utils/cx.js";

const AptitudeResultView = ({ result }) => {
    ...
    <div className="space-y-4">
      <div className="flex flex-col items-center justify-center p-6 text-center rounded-3xl border border-border bg-white shadow-card">
        ...
        <h2 className="text-xl font-black text-text-title mb-1">
          {percentage >= 80 ? "Sharpshooter!" : percentage >= 50 ? "Solid Effort!" : "Keep Training!"}
        </h2>
        <p className="text-[13px] font-semibold text-text-muted max-w-sm">
          You solved {score} out of {total} aptitude problems correctly.
        </p>
      </div>

      <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
        <ResultStat icon={<LayoutGrid className="h-4 w-4" />} label="Type" value="Aptitude" />
        ...
        <ResultStat icon={<Percent className="h-4 w-4" />} label="Percent" value={`${percentage}%`} />
      </div>
    </div>
  );
};
...
export default AptitudeResultView;
